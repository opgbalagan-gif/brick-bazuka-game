...
vm.runInNewContext(readFileSync(new URL('../web/name-input.js', import.meta.url), 'utf8'), {document, window, localStorage});
const bridge = window.BrickNameInput, input = elements.get('brick-player-name');
bridge.save_name('Игрок 42');
assert.equal(bridge.get_saved_name(), 'Игрок 42', 'The browser name must persist independently of the score API');
assert.equal(stored.get('brick-player-name'), 'Игрок 42', 'The game must share its name key with the standalone leaderboard');
