...
func run() -> void:
	...
	assert(is_zero_approx(game.player_vel.x) and not game.touch_control.is_steering(), "Mouse holds must not activate phone steering")
	move(game, Vector2(200, 500))
	game._process(0.4)
	assert(is_zero_approx(game.player_vel.x), "Mouse motion must not override phone controls")
	button(game, Vector2(200, 500), false)
	assert(not game.touch_control.is_steering(), "Mouse release must not leave a touch pointer")
	game.rockets.clear()
	game.reload_timer = 0
	...
	game.free()
	print("MOUSE_AMBIENT_TEST_OK phone_controls_not_overridden preview_click board_pause ambient_ghosts attacks_coexist")
	quit()
